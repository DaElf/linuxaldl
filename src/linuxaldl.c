...
main(int argc, const char *argv[])
{
	...
	struct poptOption aldl_opt_table[] = {
		{"serial", '\0',
		 ...
		 &aldl_settings.aldlportname, 0,
		 "Serial port the aldl interface is on",
		 "/dev/ttyUSB0"},
		{"mask", '\0',
		 POPT_ARG_STRING | POPT_ARGFLAG_ONEDASH,
		 &aldl_settings.aldldefname, 0,
	...
	poptSetOtherOptionHelp(popt_aldl,
		   ...
		   float op_offset)
{
	...
	if (operation == ALDL_OP_MULTIPLY)
		...
		result = -999;
		fprintf(stderr,
			" aldl_raw_to_float8() error: undefined operation: %d",
			operation);
	}
		    ...
		    float op_factor, float op_offset)
{
	...
	if (operation == ALDL_OP_MULTIPLY)
		...
		result = -999.0;
		fprintf(stderr,
			" aldl_raw_to_float16() error: undefined operation: %d",
			operation);
	}
	return result;
