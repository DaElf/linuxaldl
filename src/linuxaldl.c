...
linuxaldl_settings aldl_settings = {
	NULL, 0, NULL, 1, 0, NULL, NULL, aldl_definition_table, NULL, NULL, NULL, .scan_interval = 1000, .scan_timeout = 1000
};
...
main(int argc, const char *argv[])
{
	...
	// popt option table
	// -----------------
	struct poptOption aldl_opt_table[] = { { "serial", '\0', POPT_ARG_STRING | POPT_ARGFLAG_ONEDASH, &aldl_settings.aldlportname, 0,
						 "Serial port the aldl interface is on", "/dev/ttyUSB0" },
					       { "mask", '\0', POPT_ARG_STRING | POPT_ARGFLAG_ONEDASH, &aldl_settings.aldldefname, 0,
						 "ALDL code definition to use", "DF" },
					       POPT_AUTOHELP{ NULL, 0, 0, NULL, 0, 0, NULL } };
...
aldl_update_sets(int flags)
{
	...
	while (defs[i].label != NULL) // while not at the last defined byte
	{
		...
		if (cur_def->bits == 8) {
			...
			converted_val = aldl_raw8_to_float(aldl_settings.data_set_raw[byte], cur_def->operation, cur_def->op_factor,
							   cur_def->op_offset);
			...
			if (cur_def->byte_offset > 0)
				...
				byte = cur_def->byte_offset - 1;
			converted_val =
				aldl_raw16_to_float(aldl_settings.data_set_raw[byte], aldl_settings.data_set_raw[cur_def->byte_offset],
...
float
aldl_raw8_to_float(unsigned char val, int operation, float op_factor, float op_offset)
{
	float result = val;
...
// see the documentation for the byte_def_t struct in linuxaldl.h for more information
float
aldl_raw16_to_float(unsigned char msb, unsigned char lsb, int operation, float op_factor, float op_offset)
{
