...
linuxaldl_gui(int argc, char *argv[])
{
	...
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(lfilew)->ok_button), "clicked", G_CALLBACK(linuxaldl_gui_load), (gpointer)lfilew);
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(lfilew)->ok_button), "clicked", G_CALLBACK(linuxaldl_gui_widgethide),
			 (gpointer)lfilew);
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(lfilew)->cancel_button), "clicked", G_CALLBACK(linuxaldl_gui_widgethide),
			 (gpointer)lfilew);
	...
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(sfilew)->ok_button), "clicked", G_CALLBACK(linuxaldl_gui_save), (gpointer)sfilew);
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(sfilew)->ok_button), "clicked", G_CALLBACK(linuxaldl_gui_widgethide),
			 (gpointer)sfilew);
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(sfilew)->cancel_button), "clicked", G_CALLBACK(linuxaldl_gui_widgethide),
			 (gpointer)sfilew);
	// stop the window from being destroyed when the X is clicked at the top right
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(linuxaldl_gui_widgetshow), (gpointer)lfilew);
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(linuxaldl_gui_widgetshow), (gpointer)sfilew);
	gtk_container_add(GTK_CONTAINER(bbox_settings_top), button);
	gtk_widget_show(button);

	// select definition
	button = gtk_button_new_with_label("Select definition");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(linuxaldl_gui_try_choosedef), (gpointer)choosedefw);
	gtk_container_add(GTK_CONTAINER(bbox_settings_top), button);
	gtk_widget_show(button);
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(linuxaldl_gui_datareadout_show), (gpointer)datareadoutw);
...
linuxaldl_gui_scan_on_interval(gpointer data)
{
	if (aldl_settings.scanning == 0) {
		...
		send_aldl_message(aldl_settings.definition->mode9_request, aldl_settings.definition->mode9_request_length);
...
linux_aldl_start_chatter(void)
{
	...
	u_char inbuffer[128];
	int res;
	send_aldl_message(aldl_settings.definition->mode8_request, aldl_settings.definition->mode8_request_length);
...
linuxaldl_gui_options_new()
{
	...
	GtkWidget *interval_adj = hscale_new_with_label(aldl_settings.scan_interval, 50.0, 1000.0, 1.0,
							G_CALLBACK(linuxaldl_gui_scan_interval_changed), "Scan Interval (msec)");

	gtk_box_pack_start(GTK_BOX(vbox_options), interval_adj, FALSE, FALSE, 0);
	gtk_widget_show(interval_adj);

	// timeout adjustment
	GtkWidget *timeout_adj = hscale_new_with_label(aldl_settings.scan_timeout, 50.0, 1000.0, 1.0,
						       G_CALLBACK(linuxaldl_gui_scan_timeout_changed), "Scan Timeout (msec)");
...
linuxaldl_gui_datareadout_show(GtkWidget *widget, gpointer data)
{
	...
	for (i = 0; def[i].label != NULL; i++) {
		...
		if (def[i].operation == ALDL_OP_SEPERATOR) {
			...
			for (row = 0; (def[i + row + 1].label != NULL && def[i + row + 1].operation != ALDL_OP_SEPERATOR);
			     row++) { /* do nothing (just count up rows) */
			}
		...
		else // data item
		{
			...
			// add the label to the alignment, alignment to the current table
			gtk_container_add(GTK_CONTAINER(align), cur_label);
			gtk_table_attach(GTK_TABLE(cur_table), align, 1, 2, row, row + 1, GTK_FILL | GTK_EXPAND, GTK_FILL | GTK_EXPAND, 10,
					 0);
			gtk_widget_show(cur_label);
			gtk_widget_show(align);
...
linuxaldl_gui_choosedef_new()
{
	...
	gtk_entry_set_text(GTK_ENTRY(GTK_COMBO(dropdown)->entry), "No Definitions");
...
quick_alert(gchar *message)
{
	...
	GtkWidget *dialog =
		gtk_dialog_new_with_buttons("Alert", NULL, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT | GTK_DIALOG_NO_SEPARATOR,
...
GtkWidget *
hscale_new_with_label(gdouble init_val, gdouble min, gdouble max, gdouble step, GtkSignalFunc changed, gchar *adj_label)
{
	...
	g_signal_connect(G_OBJECT(new_adj), "value_changed", G_CALLBACK(changed), NULL);
