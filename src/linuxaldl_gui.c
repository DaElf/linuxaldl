...
static void aldl_gui_shutup(GtkWidget *, gpointer);
// ========================================================================
//
...
linuxaldl_gui(int argc, char *argv[])
{
	...
	gtk_button_box_set_layout(GTK_BUTTON_BOX(bbox_settings_top),
	...
	g_signal_connect(G_OBJECT(lfilew), "destroy",
			 G_CALLBACK(linuxaldl_gui_widgethide),
			 (gpointer) lfilew);
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(lfilew)->ok_button),
			 "clicked", G_CALLBACK(linuxaldl_gui_load),
			 (gpointer) lfilew);
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(lfilew)->ok_button),
			 "clicked", G_CALLBACK(linuxaldl_gui_widgethide),
			 (gpointer) lfilew);
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(lfilew)->cancel_button),
			 "clicked", G_CALLBACK(linuxaldl_gui_widgethide),
			 (gpointer) lfilew);
	// stop the window from being destroyed when the X is clicked at the top right
	g_signal_connect(G_OBJECT(lfilew), "delete_event",
			 G_CALLBACK(hide_on_delete), NULL);
	...
	g_signal_connect(G_OBJECT(sfilew), "destroy",
			 G_CALLBACK(linuxaldl_gui_widgethide),
			 (gpointer) sfilew);
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(sfilew)->ok_button),
			 "clicked", G_CALLBACK(linuxaldl_gui_save),
			 (gpointer) sfilew);
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(sfilew)->ok_button),
			 "clicked", G_CALLBACK(linuxaldl_gui_widgethide),
			 (gpointer) sfilew);
	g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(sfilew)->cancel_button),
			 "clicked", G_CALLBACK(linuxaldl_gui_widgethide),
			 (gpointer) sfilew);
	// stop the window from being destroyed when the X is clicked at the top right
	g_signal_connect(G_OBJECT(sfilew), "delete_event",
			 G_CALLBACK(hide_on_delete), NULL);
	...
	// Commands buttons (shutup, scan, stop, options, quit)
	// ========================================================================
  ...
  // shutup button
  button = gtk_toggle_button_new_with_label("shutup");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(aldl_gui_shutup), (gpointer) & aldl_settings);
	gtk_container_add(GTK_CONTAINER(bbox_cmds), button);
	gtk_widget_show(button);

	// scan button
	button = gtk_toggle_button_new_with_label("Scany");
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(linuxaldl_gui_widgetshow), (gpointer) optionsw);
	gtk_container_add(GTK_CONTAINER(bbox_cmds), button);
	gtk_widget_show(button);

	// exit button
	button = gtk_button_new_with_label("Quit");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(linuxaldl_gui_quit), NULL);
	gtk_container_add(GTK_CONTAINER(bbox_cmds), button);
	gtk_widget_show(button);
	...
	g_signal_connect(G_OBJECT(button), "clicked",
			 G_CALLBACK(linuxaldl_gui_widgetshow),
			 (gpointer) lfilew);
	...
	g_signal_connect(G_OBJECT(button), "clicked",
			 G_CALLBACK(linuxaldl_gui_try_choosedef),
			 (gpointer) choosedefw);
	gtk_container_add(GTK_CONTAINER(bbox_settings_top), button);
	gtk_widget_show(button);
	...
	gtk_box_pack_start(GTK_BOX(vbox_settings), frame_data_control, FALSE,
			   FALSE, 0);
	...
	g_signal_connect(G_OBJECT(button), "clicked",
			 G_CALLBACK(linuxaldl_gui_datareadout_show),
			 (gpointer) datareadoutw);
...
linuxaldl_gui_scan_interval_changed(GtkAdjustment * adj, gpointer data)
{
	...
	if (aldl_settings.scanning == 1) {
		...
		aldl_gui_settings.scanning_tag =
...
linux_aldl_start_chatter(void)
{
	// send a mode 8 message to silence the ecm
        ...
        res = readwithtimeout(aldl_settings.faldl, inbuffer, aldl_settings.definition->mode8_request_length, 1);
        fprintf(stderr,"%s:%d\n", __func__, __LINE__);
        fprinthex(stderr, inbuffer, aldl_settings.definition->mode8_request_length);
        usleep(1000);
...
start_mode1(void)
{
        ...
        send_aldl_message(aldl_settings.definition->mode1_request, aldl_settings.definition->mode1_request_length);
}
...
linuxaldl_gui_scan(GtkWidget * widget, gpointer data, int start_chatter)
{
  ...
  memset(inbuffer, 0 , buf_size);
	...
	tcflush(aldl_settings.faldl, TCIOFLUSH);	// flush send and receive buffers
        usleep(1000);
        /*
        res = read(aldl_settings.faldl, inbuffer, 3);
        if (res > 0) {
          fprintf(stderr, "%s:%d: res %d\t", __func__, __LINE__, res);
          fprinthex(stderr, inbuffer, 3);
        } else {
...
#if 0
  /*  fix this to be valid on checksum only */
	if (res > 0) {
		...
		if (aldl_settings.data_set_raw == NULL) {
			...
			memcpy(aldl_settings.data_set_raw,
			       inbuffer +
			       aldl_settings.definition->mode1_data_offset,
...
static void
aldl_gui_shutup(GtkWidget *widget, gpointer data)
{
  int ret;

  if (aldl_settings.definition == NULL) {
    g_print ("shutup aborted -- No definition file selected.\n");
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), FALSE);
    return;
  }

  ret = shutup();
  fprintf(stderr, "%s:%d shutup returned %d\n", __func__, __LINE__, ret);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), FALSE);
  return;
}
...
linuxaldl_gui_scan_toggle(GtkWidget * widget, gpointer data)
{
	...
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
		...
		if (aldl_settings.scanning == 0) {
			...
			aldl_gui_settings.scanning_tag = g_timeout_add(aldl_settings.scan_interval, linuxaldl_gui_scan_on_interval, NULL);
...
linuxaldl_gui_save(GtkWidget * widget, GtkFileSelection * fs)
{
	...
	g_print
	    ("Log file %s opened. ALDL data will be written to this file as it is received.\n",
	     logfilename);
	...
	for (length = 0; logfilename[length] != 0; length++) {	/* do nothing */
	}
	...
	if (0 == strncmp(".csv", extension, 4)
	    || 0 == strncmp(".CSV", extension, 4)) {
		aldl_gui_settings.log_format = ALDL_LOG_CSV;

		aldl_gui_settings.slogfile =
		    fdopen(aldl_settings.flogfile, "a+");
		...
		if (aldl_settings.definition == NULL)
			g_warning
			    (".csv format selected but no definition has been loaded.\n");
		else
...
linuxaldl_gui_write_csv_header()
{
	...
	if (aldl_gui_settings.slogfile == NULL) {
		g_warning
		    ("linuxaldl_gui_write_csv_header() invoked but no CSV file stream opened.\n");
		return;
	}
	if (aldl_settings.definition == NULL) {
		g_warning
		    ("Definition file not selected. .CSV header line could not be written.\n");
		return;
	}
	...
	for (i = 0; def[i].label != NULL; i++) {
		if (def[i].operation != ALDL_OP_SEPERATOR)
			fprintf(aldl_gui_settings.slogfile, ",%s",
				def[i].label);
	}
...
linuxaldl_gui_write_csv_line()
{
	...
	if (aldl_gui_settings.log_format != ALDL_LOG_CSV) {
		g_warning
		    ("linuxaldl_gui_write_csv_line() invoked but CSV format not selected.\n");
		return;
	} else if (aldl_gui_settings.slogfile == NULL) {
		g_warning
		    ("linuxaldl_gui_write_csv_line() invoked but no CSV file stream opened.\n");
		return;
	} else if (aldl_settings.definition == NULL) {
		g_warning
		    ("Definition file not selected. .CSV data line could not be written.\n");
		return;
	} else if (aldl_settings.data_set_strings != NULL) {
		byte_def_t *def = aldl_settings.definition->mode1_def;

		// write the timestamp
		fprintf(aldl_gui_settings.slogfile, "%d+%f",
			(int)aldl_gui_settings.data_timestamp.tv_sec,
			(float)aldl_gui_settings.data_timestamp.tv_usec /
			1000000.0);
		...
		for (i = 0; def[i].label != NULL; i++) {
			...
			if (def[i].operation != ALDL_OP_SEPERATOR)
				fprintf(aldl_gui_settings.slogfile, ",%s",
					aldl_settings.data_set_strings[i]);
		}
		fprintf(aldl_gui_settings.slogfile, "\n");	// end the line
	}
}
...
linuxaldl_gui_options_new()
{
	...
	gtk_window_set_title(GTK_WINDOW(optionsw),
			     "linuxaldl - Options & Settings");
	gtk_window_set_position(GTK_WINDOW(optionsw), GTK_WIN_POS_CENTER);

	// stop the window from being destroyed when the X is clicked at the top right
	g_signal_connect(G_OBJECT(optionsw), "delete_event",
			 G_CALLBACK(hide_on_delete), NULL);
...
linuxaldl_gui_datareadout_new()
{
	...
	// stop the window from being destroyed when the X is clicked at the top right
	g_signal_connect(G_OBJECT(dataw), "delete_event",
			 G_CALLBACK(hide_on_delete), NULL);
...
linuxaldl_gui_datareadout_update(GtkWidget * widget, gpointer data)
{
	...
	for (i = 0; defs[i].label != NULL; i++)	// while not at the last defined byte
	{
		...
		gtk_label_set_text(GTK_LABEL
				   (aldl_gui_settings.data_readout_labels[i]),
				   aldl_settings.data_set_strings[i]);
	}
}
...
linuxaldl_gui_datareadout_show(GtkWidget * widget, gpointer data)
{
	...
	if (aldl_settings.definition == NULL) {
		g_print
		    ("No definition selected. Cannot show data readout. Choose a definition first.\n");
		return;
	}
	...
	aldl_gui_settings.data_readout_labels =
	    g_malloc0((num_items) * sizeof(GtkWidget *));
	GtkWidget *cur_vbox;
	GtkWidget *cur_frame;
	GtkWidget *cur_label;
	GtkWidget *cur_table = NULL;
	...
	for (i = 0; def[i].label != NULL; i++) {
		...
		if (def[i].operation == ALDL_OP_SEPERATOR) {
			...
			gtk_box_pack_start(GTK_BOX(databox), cur_frame, FALSE,
					   FALSE, 0);
			...
			for (row = 0; (def[i + row + 1].label != NULL && def[i + row + 1].operation != ALDL_OP_SEPERATOR); row++) {	/* do nothing (just count up rows) */
			}
		...
		else		// data item
		{
			...
			gtk_table_attach_defaults(GTK_TABLE(cur_table), align,
						  0, 1, row, row + 1);
			...
			// add the label to the alignment, alignment to the current table
			gtk_container_add(GTK_CONTAINER(align), cur_label);
			gtk_table_attach(GTK_TABLE(cur_table), align, 1, 2, row,
					 row + 1, GTK_FILL | GTK_EXPAND,
					 GTK_FILL | GTK_EXPAND, 10, 0);
			gtk_widget_show(cur_label);
			gtk_widget_show(align);
...
linuxaldl_gui_choosedef_new()
{
	...
	gtk_window_set_title(GTK_WINDOW(defwindow),
			     "linuxaldl - Choose ALDL Definition");
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(linuxaldl_gui_widgethide), (gpointer) defwindow);
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(linuxaldl_gui_widgethide), (gpointer) defwindow);
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(linuxaldl_gui_load_definition), GTK_ENTRY(GTK_COMBO(dropdown)->entry));
	gtk_container_add(GTK_CONTAINER(bbox), button);
	gtk_widget_show(button);
...
linuxaldl_gui_load_definition(GtkWidget * widget, gpointer data)
{
	...
	aldl_settings.data_set_raw =
	    g_malloc0(aldl_settings.definition->mode1_data_length);
	// allocate memory for the float array
	aldl_settings.data_set_floats =
	    g_malloc0(aldl_settings.definition->mode1_data_length *
		      sizeof(float));
	// allocate memory for the string pointers array
	aldl_settings.data_set_strings =
	    g_malloc0(aldl_settings.definition->mode1_data_length *
		      sizeof(char *));
		      ...
		      gdouble step, GtkSignalFunc changed, gchar * adj_label)
{
	...
	GtkObject *new_adj =
	    gtk_adjustment_new(init_val, min, max, step, 0.0, 0.0);
	...
	gtk_range_set_update_policy(GTK_RANGE(new_scale),
