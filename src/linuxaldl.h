...
typedef enum _ALDL_OP { ALDL_OP_MULTIPLY = 0, ALDL_OP_DIVIDE =
	    1, ALDL_OP_SEPERATOR = 9
} ALDL_OP_t;
...
typedef struct _linuxaldl_byte_definition {
	const char *label;
	u_int byte_offset;	// the offset from the 1st byte of the data part
	// of the mode1 message
	u_int bits;	// 8 or 16 are currently supported

	u_int operation;	// ALDL_OP_MULTIPLY: (X*factor)+offest
        /*
...
typedef struct _linuxaldl_definition {
	const char *name;
        u_char mode1_request[__MAX_REQUEST_SIZE];	// the mode 1 request message, including the checksum
	u_int mode1_request_length;	// the length of the mode 1 message including the checksum

	u_int mode1_response_length;	// the total length of the response from the ecm

	u_int mode1_data_length;	// the number of data bytes in the mode1 message response

	u_int mode1_data_offset;	// the byte offest from the start of the mode1 message response
	...
	u_char mode8_request[__MAX_REQUEST_SIZE];	// the mode 8 (silence) request message, incl checksum
	u_int mode8_request_length;	// the length of the mode 8 message incl checksum

	u_char mode9_request[__MAX_REQUEST_SIZE];	// the mode 9 (un-silence) request message, incl checksum
	u_int mode9_request_length;	// the length of the mode 9 message including the checksum
...
u_char get_checksum(u_char *buffer, unsigned int len);
...
int send_aldl_message(u_char *msg_buf, size_t size);
...
int get_mode1_message(u_char *inbuffer, unsigned int size);
...
int aldl_listen_raw(u_char *inbuffer, unsigned int len, int timeout);
...
typedef enum _ALDL_UPDATE_FLAGS {
	ALDL_UPDATE_STRINGS = 1,
        ALDL_UPDATE_FLOATS = 2
} ALDL_UPDATE_FLAGS_t;
...
float aldl_raw8_to_float(u_char val, int operation, float op_factor,
			 float op_offset);
...
float aldl_raw16_to_float(u_char msb, u_char lsb, int operation,
